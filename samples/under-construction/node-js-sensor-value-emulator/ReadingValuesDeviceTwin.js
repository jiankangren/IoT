...
var connectionString = "HostName=oscheeriothub.azure-devices.net;DeviceId=VirtualDevice003;SharedAccessKey=JbmMzc0pJa0BNut6Mr0CyWf3sMIhrCCtrCPH+jnGdUk=";

var registry = iothub.Registry.fromConnectionString(connectionString);

var deviceId = 'VirtualDevice003';
...
var queryTwins = function() {
  var query = registry.createQuery("SELECT * FROM devices WHERE tags.location.plant = '" + deviceId + "'", 100);
  query.nextAsTwin(function(err, results) {
      if (err) {
          console.error('Failed to fetch the results: ' + err.message);
      } else {
          console.log("Devices in " + deviceId + ": " + results.map(function(twin) {return twin.deviceId}).join(','));
      }
  });
  ...
  query.nextAsTwin(function(err, results) {
      if (err) {
          ...
          console.log("Devices in " + deviceId + " using cellular network: " + results.map(function(twin) {return twin.deviceId}).join(','));
      }
  });
};
